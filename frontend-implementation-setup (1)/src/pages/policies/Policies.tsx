...
import { Plus, Search, FileCheck, Edit, Zap } from 'lucide-react';
...
const TABS = ['Policies', 'Rules'] as const;
...
export const Policies = () => {
  ...
  const openEditRule = (rule: PolicyRule) => {
    ...
    setRuleForm({
      ...
      threshold: String(rule.threshold), action: rule.action,
  ...
  const handleSaveRule = (e: React.FormEvent) => {
    ...
    if (editRule) {
      setRules((prev) => prev.map((r) => r.id === editRule.id
        ? { ...r, name: ruleForm.name, description: ruleForm.description, threshold: Number(ruleForm.threshold), action: ruleForm.action as PolicyRule['action'], claimType: ruleForm.claimType || undefined } : r
      ));
      success('Rule updated', `"${ruleForm.name}" has been updated.`);
    } else {
      const newRule: PolicyRule = {
        id: `rule_${Date.now()}`,
        name: ruleForm.name, description: ruleForm.description,
        threshold: Number(ruleForm.threshold), action: ruleForm.action as PolicyRule['action'],
        claimType: ruleForm.claimType || undefined, isActive: true,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Manage insurance policies and processing rules.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {tab === 'Policies' && (
            <Button size="sm" leftIcon={<Plus size={14} />} onClick={() => setIsPolicyModalOpen(true)}>
              Add Policy
            </Button>
          )}
          {tab === 'Rules' && (
            <Button size="sm" leftIcon={<Plus size={14} />} onClick={() => { setEditRule(null); setIsRuleModalOpen(true); }}>
              Add Rule
            </Button>
          )}
        </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span className={cn('px-2 py-0.5 rounded text-[10px] font-bold uppercase border', actionStyle[rule.action])}>
                      {rule.action}
                    </span>
                    {rule.claimType && (
                      <span className="text-[10px] text-gray-600 font-mono bg-white/[0.04] px-2 py-0.5 rounded border border-white/[0.06]">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className={cn(
                      ...
                      rule.isActive ? 'bg-[#10B981]' : 'bg-white/[0.10]'
                    )}
                  >
                    <span className={cn(
                      'absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow transition-all duration-200',
      ...
      <Modal isOpen={isPolicyModalOpen} onClose={() => setIsPolicyModalOpen(false)} title="Add Policy" size="md">
        <form onSubmit={handleAddPolicy} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Input label="Holder Name" placeholder="John Smith" value={policyForm.holderName}
              onChange={(e) => setPolicyForm({ ...policyForm, holderName: e.target.value })} required />
            <Input label="Holder Email" type="email" placeholder="john@email.com" value={policyForm.holderEmail}
              onChange={(e) => setPolicyForm({ ...policyForm, holderEmail: e.target.value })} required />
          </div>
          <Select label="Policy Type" options={TYPE_OPTIONS} value={policyForm.type}
            onChange={(e) => setPolicyForm({ ...policyForm, type: e.target.value })} />
          <div className="grid grid-cols-3 gap-4">
            <Input label="Coverage ($)" type="number" placeholder="250000" value={policyForm.coverageAmount}
              onChange={(e) => setPolicyForm({ ...policyForm, coverageAmount: e.target.value })} required />
            <Input label="Premium/yr ($)" type="number" placeholder="1200" value={policyForm.premium}
              onChange={(e) => setPolicyForm({ ...policyForm, premium: e.target.value })} required />
            <Input label="Deductible ($)" type="number" placeholder="1000" value={policyForm.deductible}
              onChange={(e) => setPolicyForm({ ...policyForm, deductible: e.target.value })} required />
          </div>
      ...
      <Modal
        ...
        onClose={() => { setIsRuleModalOpen(false); setEditRule(null); }}
        title={editRule ? 'Edit Rule' : 'Add Rule'}
        size="md"
      >
        <form onSubmit={handleSaveRule} className="space-y-4">
          <Input label="Rule Name" placeholder="High Value Auto Flag" value={ruleForm.name}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input label="Threshold" type="number" placeholder="25000" value={ruleForm.threshold}
              onChange={(e) => setRuleForm({ ...ruleForm, threshold: e.target.value })} required />
            <Select label="Action" options={ACTION_OPTIONS} value={ruleForm.action}
              onChange={(e) => setRuleForm({ ...ruleForm, action: e.target.value })} />
          </div>
          <Input label="Claim Type (optional)" placeholder="auto_collision" value={ruleForm.claimType}
            onChange={(e) => setRuleForm({ ...ruleForm, claimType: e.target.value })} />
          <div className="flex justify-end gap-3 pt-2">
