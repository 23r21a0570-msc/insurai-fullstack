import { Claim, ClaimStatus, ClaimType, RiskLevel, RiskFactor, AIAnalysis, Policy, PolicyType, PolicyStatus, User, DashboardStats, ClaimsTrendData, RiskDistribution, RecentActivity, FraudAlert, AuditLogEntry, ClaimNote, AppNotification, PolicyRule } from '@/types';
...
const generateRiskFactors = (riskScore: number): RiskFactor[] => {
  ...
  if (riskScore > 20) factors.push({ id: rid(), name: 'High Claim Amount', description: 'Amount is above average for this claim type', impact: riskScore > 60 ? 'high' : 'medium', score: Math.floor(riskScore * 0.25), category: 'financial' });
  if (riskScore > 40) factors.push({ id: rid(), name: 'Multiple Recent Claims', description: 'Several claims filed in the past two years', impact: 'medium', score: Math.floor(riskScore * 0.2), category: 'historical' });
  if (riskScore > 60) factors.push({ id: rid(), name: 'Document Mismatch', description: 'Dates or amounts in documents do not match', impact: 'high', score: Math.floor(riskScore * 0.25), category: 'document' });
  if (riskScore > 75) factors.push({ id: rid(), name: 'Recent Policy Change', description: 'Claim filed shortly after coverage was updated', impact: 'high', score: Math.floor(riskScore * 0.3), category: 'behavioral' });
  return factors;
};
...
export const mockCustomerPolicies: CustomerPolicy[] = [
  {
    ...
    endDate: new Date(Date.now() + 185 * 86400000).toISOString(),
    nextPaymentDate: new Date(Date.now() + 15 * 86400000).toISOString(),
    nextPaymentAmount: 1200,
  },
  {
    id: 'cpol_2',
    policyNumber: 'POL-4392',
    type: 'home',
    status: 'active',
    ...
    endDate: new Date(Date.now() + 275 * 86400000).toISOString(),
    nextPaymentDate: new Date(Date.now() + 22 * 86400000).toISOString(),
    nextPaymentAmount: 2400,
  },
  {
    id: 'cpol_3',
    policyNumber: 'POL-1157',
    type: 'health',
    status: 'pending',
    ...
    nextPaymentDate: new Date(Date.now() + 10 * 86400000).toISOString(),
...
export const mockCustomerClaims: CustomerClaim[] = [
  {
    ...
    description: 'Rear-end collision at traffic light. Bumper and trunk damage.',
  },
  {
    id: 'cclaim_2',
    ...
    description: 'Stolen vehicle accessories. Police report filed.',
  },
  {
    id: 'cclaim_3',
    claimNumber: 'CLM-2024-0018',
    policyNumber: 'POL-4392',
    type: 'property_damage',
    status: 'pending_info',
    amount: 12000,
    submittedAt: new Date(Date.now() - 35 * 86400000).toISOString(),
    updatedAt: new Date(Date.now() - 28 * 86400000).toISOString(),
    description: 'Water damage from burst pipe in kitchen.',
  },
  {
    id: 'cclaim_4',
    claimNumber: 'CLM-2023-0094',
    policyNumber: 'POL-7821',
    type: 'auto_collision',
    status: 'rejected',
    amount: 1500,
    ...
    description: 'Minor fender scratch — below deductible threshold.',
  },
];
...
export const mockCustomerPayments: CustomerPayment[] = [
  ...
  },
  {
    ...
    status: 'paid',
    paidAt: new Date(Date.now() - 20 * 86400000).toISOString(),
  },
];

const policyTypes: PolicyType[] = ['auto', 'home', 'health', 'life', 'business'];
...
export const mockPolicies: Policy[] = Array.from({ length: 30 }, (_, i) => {
  ...
  return {
    id: `policy_${i + 1}`,
    policyNumber: `POL-${String(Math.floor(Math.random() * 9000) + 1000)}`,
    type, holderName: name,
    holderEmail: `${name.toLowerCase().replace(' ', '.')}@email.com`,
    coverageAmount: Math.floor(Math.random() * 500000) + 100000,
    premium: Math.floor(Math.random() * 3000) + 500,
    deductible: Math.floor(Math.random() * 2000) + 500,
    startDate: new Date(Date.now() - Math.random() * 365 * 86400000).toISOString(),
    endDate: new Date(Date.now() + Math.random() * 365 * 86400000).toISOString(),
    status, claimsCount: Math.floor(Math.random() * 5),
    totalClaimsAmount: Math.floor(Math.random() * 50000),
  };
});
...
export const getDashboardStats = (): DashboardStats => {
  ...
  return {
    totalClaims: total, claimsChange: 12.5,
    pendingReview: pending, pendingChange: -5.2,
    approvalRate: Math.round((approved / total) * 100), approvalChange: 3.1,
    avgProcessingTime: 2.4, processingChange: -15.3,
    fraudDetected: highRisk, fraudChange: 8.7,
    totalPayout: totalAmount, payoutChange: 22.4,
  };
};
...
export const getAuditLog = (): AuditLogEntry[] => [
  ...
  { id: '2', action: 'login', entityType: 'user', entityId: 'user_1', userId: 'user_1', userName: 'Alex Thompson', userRole: 'admin', description: 'Administrator login from new device', timestamp: new Date(Date.now() - 1200000).toISOString() },
  ...
  { id: '6', action: 'create', entityType: 'user', entityId: 'user_6', userId: 'user_1', userName: 'Alex Thompson', userRole: 'admin', description: 'Created new agent account for Priya Patel', timestamp: new Date(Date.now() - 86400000).toISOString() },
];

export const getClaimById = (id: string): Claim | undefined => mockClaims.find(c => c.id === id);
export const getPolicyById = (id: string): Policy | undefined => mockPolicies.find(p => p.id === id);

export const getClaimNotes = (claimId: string): ClaimNote[] => [
  { id: 'note_1', claimId, authorId: 'user_2', authorName: 'Sarah Chen', authorRole: 'manager', content: 'Spoke with the claimant. They confirmed the incident occurred near Highway 9. Repair shop estimate looks consistent with the reported damage.', createdAt: new Date(Date.now() - 3600000 * 5).toISOString() },
  { id: 'note_2', claimId, authorId: 'user_3', authorName: 'Mike Ross', authorRole: 'analyst', content: 'Cross-referenced with police report. Incident date and location match. AI risk score elevated due to claim amount — but after manual review, seems legitimate.', createdAt: new Date(Date.now() - 3600000 * 2).toISOString() },
  { id: 'note_3', claimId, authorId: 'user_4', authorName: 'Emily Wang', authorRole: 'analyst', content: 'Requested additional photos from the claimant. Awaiting response before final decision.', createdAt: new Date(Date.now() - 1800000).toISOString() },
];
...
export const getNotifications = (): AppNotification[] => [
  ...
  { id: 'n4', type: 'info', title: 'AI Analysis Complete', message: '15 new claims analyzed by AI model', read: true, link: '/claims', createdAt: new Date(Date.now() - 7200000).toISOString() },
